...
from traffic_adv.methods.mmad.utils import *
...
class GaussianDiffusion1D(Module):
    ...
    def p_mean_variance(self, x, t, x_self_cond = None, clip_denoised = True, model_forward_kwargs: dict = dict()):
        preds = self.model_predictions(x, t, x_self_cond=x_self_cond, model_forward_kwargs=model_forward_kwargs)
    ...
    def p_sample_loop(self, shape, return_noise = False, model_forward_kwargs: dict = dict()):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            ...
            img, x_start = self.p_sample(img, t, self_cond, clip_denoised = False,
                                         model_forward_kwargs = model_forward_kwargs)
    ...
    @torch.no_grad()
    def sample(self, batch_size = 16, mask=None, return_noise = False, model_forward_kwargs: dict = dict()):
    ...
    def cond_fn(classifier, x, t, y = None):
        '''
        '''
        with torch.enable_grad():
            ...
            log_probs = torch.log(1 - probs + 1e-8)  # log(p(y=0))
    ...
    def forward(self, img, *args, **kwargs):
        ...
        model_kwargs = kwargs.pop('model_forward_kwargs', {})

        model_kwargs.update(kwargs)
        
        return self.p_losses(img, t, model_forward_kwargs=model_kwargs, *args)
